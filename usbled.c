			 ...
			 const char *buf, size_t count)
{
	...
	if (retval) {
		dev_err(&led->udev->dev, "Could not submit interrupt_out_urb%d\n", retval);
		return retval;
	}
...
{
	struct usb_led *dev = urb->context;

	dev_info(&dev->udev->dev, "led_urb_out_callback() function is called.\n");

	/* sync/async unlink fault are not errors */
	if (urb->status) {
...
static int led_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	ep = usb_endpoint_num(endpoint);	/* value form 0 to 15, it is 1 */
	size = usb_endpoint_maxp(endpoint);

	/* Validate endpoint and size */
	if (size <= 0) {
		dev_info(&intf->dev, "invalid size (%d)\n", size);
		return -ENODEV;
	}
	...
	if (retval) {
		dev_err(&dev->udev->dev,
			"Could not submit interrupt_in_urb %d\n", retval);
...
static void led_disconnect(struct usb_interface *intf)
{
	...
	dev_info(&intf->dev, "USB LED now disconnected\n");
}
...
MODULE_DESCRIPTION("This is a led/switch usb controlled module with irq in/out endpoint");
